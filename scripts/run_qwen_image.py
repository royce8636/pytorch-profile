...
from profile_qwen_image_common import (  # noqa: E402
    DTYPE_BY_NAME,
    build_generator,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Qwen-Image without profiler instrumentation."
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Qwen transformer forward with torch.compile."
        ),
    )
    parser.add_argument(
        "--compile-mode",
...
def cuda_unavailable_error(device: torch.device) -> str:
    ...
    if not torch.backends.cuda.is_built():
        lines.append(
            "This Python process is using a CPU-only PyTorch build, so the Qwen runner cannot use the GPU."
        )
    else:
        lines.append(
